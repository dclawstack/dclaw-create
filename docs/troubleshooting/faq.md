...
### How do I update DClaw Create?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp create --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Create without Kubernetes?

Yes, for local development:

```bash
cd dclaw-create/frontend && npm run dev
cd dclaw-create/backend && uvicorn dclaw_create.main:app --reload
```
