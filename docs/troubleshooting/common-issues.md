...
# Check events
kubectl get events -n dclaw-create --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-create deployment/dclaw-create-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-create-db-credentials -n dclaw-create
   ```
